# Squad Team

> NeoTetris

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Flynn | Lead | .squad/agents/flynn/charter.md | 🏗️ Lead |
| Quorra | Frontend Dev | .squad/agents/quorra/charter.md | ⚛️ Frontend |
| Tron | Game Dev | .squad/agents/tron/charter.md | 🔧 Game Dev |
| Rinzler | Tester | .squad/agents/rinzler/charter.md | 🧪 Tester |
| Scribe | Scribe | .squad/agents/scribe/charter.md | 📋 Scribe |
| Ralph | Work Monitor | — | 🔄 Monitor |

## Project Context

- **Project:** NeoTetris — A Tetris game in pure HTML/CSS/JS with neo-brutalist design
- **User:** Brady
- **Stack:** HTML, CSS, JavaScript (zero dependencies)
- **Created:** 2026-04-16
